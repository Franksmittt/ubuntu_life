...
(function ($) {
	...
	function gsapController() {
				...
				btn.addEventListener("click", e => {
					...
					const smoother =
						typeof ScrollSmoother !== "undefined"
							? ScrollSmoother.get()
							: null;
					if (smoother && sectionTarget && sectionTarget.charAt(0) === "#") {
						const el = document.querySelector(sectionTarget);
						if (el) {
							smoother.scrollTo(el, true, "top 120");
							return;
						}
					}
					gsap.to(window, {
		...
		// Deep links (#section): native hash scroll does not move ScrollSmoother's virtual scroll.
		function tjScrollToHashAnchor() {
			...
			var run = function () {
				...
				if (smoother && typeof smoother.scrollTo === "function") {
					smoother.scrollTo(el, false, "top 120");
				} else {
		...
		tjScrollToHashAnchor();
		$(window).on("hashchange", tjScrollToHashAnchor);
	}
